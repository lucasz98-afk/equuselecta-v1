...
import { Menu, X, ArrowUpRight, Instagram, Globe, Mail } from 'lucide-react';
import { HorseCategory } from '../types';

/**
 * WORDPRESS ACF MAPPING:
 * ---------------------
 * Global Options Page (Theme Settings):
 * - Logo Image or Text (text/image)
 * - Navigation Links (Repeater):
 *    - Label (text)
 *    - Link URL (url)
 *    - Number/Index (text) - e.g. "01"
 * - Social Media (Repeater): Name, URL, Icon Class.
 * - Contact Info: Email, Address.
 * 
 * Z-INDEX LAYERS:
 * - Floating Bar: z-50
 * - Curtain Menu: z-100 (Full overlay)
 */
...
export const Navbar: React.FC<NavbarProps> = ({ onSelectCategory, activeCategory, onSellClick }) => {
  ...
  // WP TRANSLATION: CSS Class .nav-curtain defined in animations.css
  const menuVariants = {
    initial: { 
  ...
  const linkContainerVars = {
    ...
    animate: (i: number) => ({
      opacity: 1,
      y: 0,
      transition: { duration: 0.7, ease: [0.22, 1, 0.36, 1] as [number, number, number, number], delay: 0.3 + (i * 0.1) }
    }),
      ...
      */}
      <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 w-full flex justify-center pointer-events-none px-4 transition-all duration-500 ${isMenuOpen ? 'opacity-0 -translate-y-10' : 'opacity-100 translate-y-0'}`}>
        ...
        >
          ...
          {/* WP: Generate these from Taxonomy terms loop or hardcoded menu */}
      ...
      <AnimatePresence mode="wait">
        {isMenuOpen && (
          ...
          >
            ...
            <div className="relative z-10 flex-1 flex flex-col justify-center px-6 md:px-20 lg:px-32">
               <div className="flex flex-col items-start gap-2">
                  {NAV_ITEMS.map((item, idx) => (
                    ...
                    >
                      {/* BLUR EFFECT LOGIC - WP: Use CSS :has() or JS mouseover */}
                      <motion.div
                        animate={{
            ...
            {/* --- INTERNAL FOOTER --- */}
            {/* WP: Use ACF Options Page data here */}
